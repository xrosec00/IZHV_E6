        ...
        { // Main Window Area
            ...
            GUILayout.BeginVertical();
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    var interact = GUILayout.Toggle(GameManager.Instance.interactiveMode, "",GUILayout.Width(WINDOW_DIMENSION.x / 4.0f));
                    if (GUI.changed)
                    {
                        Debug.Log("Interact 2: " + interact);
                ...
                GUILayout.BeginHorizontal();
                {
                    ...
                    var master_vol = SoundManager.Instance.masterVolume;
                    master_vol = (int) GUILayout.HorizontalSlider(master_vol, -80.0f, 20.0f, 
                        GUILayout.ExpandWidth(true));
                    if (GUI.changed)
                    { SoundManager.Instance.masterVolume = master_vol; }
                    var btn = GUILayout.Button("M", GUILayout.Width(WINDOW_DIMENSION.x / 4.0f));
                    if (btn)
                    {
